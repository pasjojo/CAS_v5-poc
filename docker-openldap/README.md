...
Personnalisation de l'image dans la branche apca. La branche stable est reprise du projet sur github.

## Création du projet dans le repo lilo

```
git clone https://github.com/osixia/docker-openldap/

git remote add lilo ssh://git@gitlab.libre-logic.fr/apca/docker-openldap

git push lilo
```

La branche apca contient les éléments de personnalisation du projet APCA :

* Modification du Makefile pour y ajouter UID et GID lors du build
* Ajustement des logs dans startup.sh 
* Ajout des schémas
* Ajout d'une fonctionnalité de peuplement de la base au démarrage

Monter le volume `/container/service/slapd/prepopulate/` pour initialiser la base ldap au premier démarrage du conteneur.

Dans une composition par exemple :

```
    volumes:
    - /opt/docker/ldap/data:/var/lib/ldap
    - /opt/docker/ldap/etc/:/etc/ldap/slapd.d
    - /opt/docker/ldap/prepopulate:/container/service/slapd/prepopulate
```
