...
if [ ! -e "$FIRST_START_DONE" ]; then
  ...
  function ldap_add_or_modify (){
    local LDIF_FILE=$1
    local BIND_DN=${2:-cn=admin,$LDAP_BASE_DN}
    sed -i "s|{{ LDAP_BASE_DN }}|${LDAP_BASE_DN}|g" $LDIF_FILE
    sed -i "s|{{ LDAP_BACKEND }}|${LDAP_BACKEND}|g" $LDIF_FILE
    if grep -iq changetype $LDIF_FILE ; then
        ldapmodify -Y EXTERNAL -Q -H ldapi:/// -f $LDIF_FILE 2>&1 | log-helper info || ldapmodify -h localhost -p 389 -D $BIND_DN -w $LDAP_ADMIN_PASSWORD -f $LDIF_FILE 2>&1 | log-helper info
    else
        ldapadd -Y EXTERNAL -Q -H ldapi:/// -f $LDIF_FILE |& log-helper info || ldapadd -h localhost -p 389 -D ${BIND_DN} -w $LDAP_ADMIN_PASSWORD -f $LDIF_FILE 2>&1 | log-helper info
    fi
  }
...
EOF
  ...
  if [ "${KEEP_EXISTING_CONFIG,,}" == "true" ]; then
    ...
    if $BOOTSTRAP; then
      ...
      log-helper info "Prepopulate with ldif..."
      for f in $(find ${CONTAINER_SERVICE_DIR}/slapd/prepopulate -type f -name \*.ldif  | sort); do
        log-helper info "Processing file ${f}"
        ldap_add_or_modify "$f" ${LDAP_PREPOPULATE_BIND_DN}
      done
